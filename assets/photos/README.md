...
- `node tools/mkbanner.js` — 사진 없는 곳에 유형별 배너를 그린다.
  `--clean` 은 배너(.svg)만 지운다. 사진(.jpg)은 건드리지 않는다.
- `node tools/resize.js` — 사진을 폭 600px·236:104 로 줄인다(받은 직후 실행).

### 배너 (.svg)

식당 64곳·숙소 24곳은 공공사진이 없다. 가게 사진은 초상권·상표가 얽히고,
"서면 모텔"·"광안리 게스트하우스" 는 아예 **가상 시설**이라 받을 사진이 없다.
그래서 유형별 그림을 그려 넣는다 — 이름에서 무엇을 파는 집인지 알아내
면·국수 / 국밥·탕 / 회·해산물 / 고기·구이 / 카페 / 호텔 … 로 나눈다.

**사진이 배너보다 우선한다.** 팝업은 `<id>.jpg` 를 먼저 찾고, 없으면 `<id>.svg` 로
갈아탄다. 나중에 진짜 사진을 넣으면 배너는 자동으로 밀려난다(지울 필요 없다).

### 저작권 표기

CC BY 계열은 저작자·라이선스 표시가 **의무**다. 팝업 사진 아래에 자동으로 뜬다
(`© 저작자 · 라이선스` — 원본 페이지로 링크). 내역은 `credits.json` 에 있다.
배너는 직접 그린 그림이라 표기가 없다.
