...
```bash
node tools/all.js          # 스위트 12종 전부 (selftest/stam/buff/sub/dow/ret/day/rule/subway/bus/transit/balance)
```
...
- 후쿠오카 지하철 `data/fukuoka-subway.js`: kuwayamamasayuki/GTFS-FukuokaCitySubway(MIT)
  ...
  - 그래서 후쿠오카 **버스는 여전히 근사**다. 노코노시마(6.8km)·우미노나카미치(7.8km)는
    travel() 이 버스 접근으로 환산해 계산한다.
