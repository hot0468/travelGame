...
```bash
node tools/all.js          # 스위트 11종 전부 (selftest/stam/buff/sub/dow/ret/day/rule/subway/bus/balance)
```
...
- 이동: 도보4.5/버스18/지하철26/택시24/렌터카28 km/h. 출퇴근(07:30-09:30, 18:00-20:00) 버스·택시·
  ...
  버스 **소요시간**은 여전히 직선×1.35 근사다(노선 데이터는 번호 안내에만 쓴다).
- **버스 노선 안내**: `data/bus.js` 로 장소별 경유 노선을 들고 있다가 두 장소의 **교집합**을
  직통 노선으로 낸다(`busPair(a,b)`). 실제 경로 탐색이 아니라 집합 교집합이라
  **환승 조합은 못 찾는다** — 직통이 없으면 "환승 필요" 로만 표시한다. 직통이 나오는 쌍은 21%.
  지하철과 마찬가지로 **표시 전용**이다. 재생성은 `node tools/mkbus.js` (`.apikey` 에
  `BUSAN_BIMS_KEY=...`, 응답은 임시폴더 캐시라 재실행은 오프라인으로 된다).
...
- **지하철 노선 안내**: `data/subway.js` 로 실제 1~4호선 경로·환승을 낸다(`subwayPath(a,b)`).
  양 끝의 최근접역을 잡아 역|노선 그래프에서 다익스트라 — 인접역 1, 환승 `TRANSFER_COST`(=5)정거장.
  5로 둔 이유: 3이면 부산역→해운대를 연산·수영 2회 환승으로 잡는다(정거장은 적지만 아무도 안 그런다).
  **표시 전용이다** — `travel()` 소요시간에는 안 쓴다. 쓰려면 par 6개 재조정이 따라온다.
